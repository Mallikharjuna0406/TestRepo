...
public class ProductCompositParallelRunner {

 // 1. Setup method -> @BeforeAll --> Mock Server & Mock Client
 // 2. Tear Down -> @AfterAll --> Shutdown the mock server & mock client
 // 3. Test method -> @Test (junit) --> Parallel runner to run the feature file
 // 4. MockProductService -> Intercept the request going to product service and respond with mock response
 // 5. MockReviewService -> Intercept the request going to review service and respond with mock response
